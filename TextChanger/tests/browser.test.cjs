...
for (const _name of BROWSERS) {
  test(`${_name}: offline browser regression`, async (_suite) => {
    ...
    await _suite.test('file opens offline and all five conversions update', async _test => {
      ...
      ASSERT.equal(await _tab.locator('.copy-btn:disabled').count(), 5);
      ASSERT.deepEqual(await _tab.locator('.output-area').evaluateAll(_outputs => _outputs.map($output => $output.value)), ['', '', '', '', '']);
    });
    ...
    await _suite.test('corrupt storage and wrong value types recover without errors', async _test => {
      const _tab = await _page(_test);
      for (const _raw of ['{bad', 'null', '[]', '3', '{"input":4,"theme":"unknown","pgTableName":[],"numberOffset":{}}']) {
    ...
    await _suite.test('modern clipboard success shows feedback and edits clear it', async _test => {
      ...
      await _tab.waitForFunction(_selector => document.querySelector(_selector).classList.contains('copied'), COPY_SELECTOR);
    ...
    await _suite.test('fallback success restores focus, selection and scroll', async _test => {
      const _tab = await _page(_test, () => {
        Object.defineProperty(navigator, 'clipboard', { value: { writeText() { throw new Error('Denied'); } } });
      ...
      const _state = await _tab.evaluate(async _selector => {
        ...
        return { active: document.activeElement.id, input: [$input.selectionStart, $input.selectionEnd], output: [$output.selectionStart, $output.selectionEnd, $output.selectionDirection], copied: qs(_selector).classList.contains('copied') };
      }, COPY_SELECTOR);
    ...
    await _suite.test('false and throwing fallback never report success or leak rejection', async _test => {
      ...
      for (const _mode of ['false', 'throw']) {
        await _tab.evaluate(_mode => { document.execCommand = () => { if (_mode === 'throw') throw new Error('Blocked'); return false; }; }, _mode);
        await _tab.locator(COPY_SELECTOR).click();
    ...
    await _suite.test('failed saves retry after storage becomes available', async _test => {
      ...
      await _tab.evaluate(() => window.dispatchEvent(new Event('pagehide')));
      await _tab.evaluate(() => { window._blocked = false; window.dispatchEvent(new Event('pagehide')); });
      await _tab.reload();
    ...
    await _suite.test('parameter changes only recalculate dependent results', async _test => {
      ...
      await _tab.evaluate(() => {
        ...
        for (const [_name, _key] of [['_getLines', 'lines'], ['_parseTabularInput', 'table'], ['_convertNumberAdjustment', 'number']]) {
          ...
          window[_name] = (..._args) => { window._calls[_key] += 1; return _original(..._args); };
        }
      });
    ...
    await _suite.test('pageshow repairs restored output fields even when the input did not change', async _test => {
      ...
      await _tab.evaluate(() => {
        document.querySelectorAll('.output-area').forEach($output => { $output.value = 'stale browser snapshot'; });
        window.dispatchEvent(new Event('pageshow'));
      });
    ...
    await _suite.test('pageshow synchronizes a restored theme switch with every theme surface', async _test => {
      ...
      ASSERT.deepEqual(await _tab.evaluate(() => [document.documentElement.style.colorScheme, document.documentElement.dataset.theme, document.body.dataset.mode]), ['dark', 'dark', 'dark']);
    });

    await _suite.test('restored values without input events are saved on idle and immediate departure', async _test => {
      const _tab = await _page(_test);
      await _tab.evaluate(() => {
        document.querySelector('#input-area').value = 'restored without events';
        window.dispatchEvent(new Event('pageshow'));
      });
      await _tab.waitForFunction(_key => JSON.parse(localStorage.getItem(_key))?.input === 'restored without events', STORAGE_KEY);
      await _tab.evaluate(() => {
    ...
    await _suite.test('fallback restores the direction of a backward page-text selection', async _test => {
      ...
      const _state = await _tab.evaluate(async _selector => {
        ...
        const _before = _selection.toString();
        await _copyResult(document.querySelector(_selector));
        return { text: _selection.toString(), before: _before, anchor: _selection.anchorOffset, focus: _selection.focusOffset, sameNodes: _selection.anchorNode === _node && _selection.focusNode === _node, active: document.activeElement.tagName };
      }, COPY_SELECTOR);
      ASSERT.deepEqual(_state, { text: _state.before, before: _state.before, anchor: 5, focus: 1, sameNodes: true, active: 'BODY' });
    });

    await _suite.test('IME candidates defer conversion and committing updates all affected results once', async _test => {
      const _tab = await _page(_test);
      await _tab.locator('#input-area').fill('id\n1');
    ...
    await _suite.test('copying reconciles input changed without an input event', async _test => {
      ...
      await _tab.evaluate(() => { document.querySelector('#input-area').value = 'current'; });
      await _tab.locator(COPY_SELECTOR).click();
      ASSERT.deepEqual(await _tab.evaluate(() => window._written), ["in ('current');"]);
    });

    await _suite.test('invalid parameters expose their state and recover without additional controls', async _test => {
      const _tab = await _page(_test);
      const _count = await _tab.locator('*').count();
      await _tab.locator('#input-area').fill('id\n1');
    ...
    await _suite.test('copy feedback is described accessibly and unchanged labels are not announced again', async _test => {
      const _tab = await _page(_test, () => {
      ...
      const _changes = await _tab.evaluate(async () => {
        let _changes = 0;
        const _observer = new MutationObserver(_records => { _changes += _records.length; });
        document.querySelectorAll('.copy-label').forEach($label => _observer.observe($label, { childList: true, characterData: true, subtree: true }));
    ...
    await _suite.test('an invalid table name stops before parsing a large input', async _test => {
      ...
      await _tab.evaluate(() => {
        ...
        window._parseTabularInput = (..._args) => { window._parseCalls += 1; return _parse(..._args); };
      });
    ...
    await _suite.test('native fallback selects exact output and dispatches copy without changing the clipboard', async _test => {
      const _tab = await _page(_test, () => {
        Object.defineProperty(navigator, 'clipboard', { value: undefined });
        window._copyEvents = [];
        document.addEventListener('copy', _event => {
          ...
          // Observe the real browser event but cancel its system-clipboard write.
    ...
    await _suite.test('100,000 rows retain the final record and parameter changes stay fast', async _test => {
      ...
      const _result = await _tab.evaluate(() => {
        ...
        return { convert: _convert, adjust: performance.now() - _adjustStart, count: (_sql.match(/^  \(/gm) || []).length, last: _sql.includes("('99999', 'row99999')"), adjusted: qs('#output-number-adjust').value.endsWith('100000\trow100000') };
      });
      ...
      _test.diagnostic(`100,000 rows: ${_result.convert.toFixed(1)} ms; adjustment only: ${_result.adjust.toFixed(1)} ms`);
    });
    ...
    await _suite.test('10,000 rows convert completely within a bounded time', async _test => {
      ...
      const _result = await _tab.evaluate(() => {
        ...
        const _sql = qs('#output-pg-sql').value;
        return { elapsed: _elapsed, rows: (_sql.match(/^  \(/gm) || []).length, last: _sql.includes("('9999', 'row9999')") };
      });
