...
async function _page(_test, _name, _init) {
  ...
  _tab.on('console', _message => { if (_message.type() === 'error') _errors.push(_message.text()); });
...
for (const _name of BROWSERS) {
  test(`${_name}: R16 committed input recovers a missing compositionend without waiting for blur`, async _test => {
    ...
    for (const [_id, _before, _after, _output] of [
      ...
      const _result = await _tab.evaluate(({ _id, _after, _output }) => {
        ...
        window._convertAll = () => { _conversions += 1; _convert(); };
        ...
        // An unrelated ordinary input must not finish this control's composition.
        document.querySelector('#number-offset').dispatchEvent(new InputEvent('input', { bubbles: true, isComposing: false }));
        ...
        // A late end notification must not repeat an already committed conversion.
        $control.dispatchEvent(new CompositionEvent('compositionend', { bubbles: true }));
        window._convertAll = _convert;
        return { before: _before, during: _during, duringConversions: _duringConversions,
          after: _afterCommit, conversions: _conversions, active: document.activeElement.id };
      }, { _id, _after, _output });
      ...
      await _tab.waitForFunction(({ _key, _field, _after }) => JSON.parse(localStorage.getItem(_key))?.[_field] === _after,
  ...
  test(`${_name}: R01 copied output is reconciled even when only output fields were restored`, async _test => {
    const _tab = await _page(_test, _name, () => {
      window._written = [];
      Object.defineProperty(navigator, 'clipboard', { value: { writeText: async _text => window._written.push(_text) } });
    });
    await _tab.locator('#input-area').fill('id\n1');
    const _expected = await _tab.locator('.output-area').evaluateAll(_outputs => _outputs.map($output => $output.value));
    await _tab.evaluate(() => document.querySelectorAll('.output-area').forEach($output => { $output.value = 'stale restored output'; }));
    await _tab.locator(COPY_SELECTOR).click();
    ASSERT.deepEqual(await _tab.evaluate(() => window._written), [_expected[2]]);
    ASSERT.deepEqual(await _tab.locator('.output-area').evaluateAll(_outputs => _outputs.map($output => $output.value)), _expected);
  });
  ...
  test(`${_name}: R02 pending copy rechecks eventless source changes before fallback and feedback`, async _test => {
    const _tab = await _page(_test, _name, () => {
      ...
      Object.defineProperty(navigator, 'clipboard', { value: { writeText: () => new Promise((_resolve, _reject) => window._pending.push({ _resolve, _reject })) } });
      document.execCommand = () => { window._fallbackCalls += 1; return true; };
    });
    for (const [_field, _value, _target] of [['input-area', 'id\n2', '#output-sql-in'], ['pg-table-name', 'updated', '#output-pg-sql'], ['number-offset', '5', '#output-number-adjust']]) {
      for (const _action of ['_resolve', '_reject']) {
        ...
        await _tab.evaluate(([_field, _value, _action]) => {
  ...
  test(`${_name}: R03 hidden pages cancel copies and visible pages reconcile restored controls`, async _test => {
    ...
    for (const _action of ['_resolve', '_reject']) {
      ...
      ASSERT.equal(await _tab.locator('.copy-btn.copied').count(), 0);
      ASSERT.equal(await _tab.evaluate(() => window._fallbackCalls), 0);
      await _tab.evaluate(() => { window._visibility = 'visible'; document.dispatchEvent(new Event('visibilitychange')); });
    }
    await _tab.evaluate(() => {
      window._visibility = 'hidden';
      document.dispatchEvent(new Event('visibilitychange'));
  ...
  test(`${_name}: R04 composition belongs to its control and blur recovers a missing compositionend`, async _test => {
    ...
    const _states = await _tab.evaluate(() => {
      ...
      $table.dispatchEvent(new CompositionEvent('compositionend', { bubbles: true }));
      return { unrelatedEnd: _unrelatedEnd, afterBlur: _afterBlur, sql: _sql, lateEnd: _lateEnd, final: document.querySelector('#output-pg-sql').value };
    });
  ...
  test(`${_name}: R05 change-only edits update all controls and save without duplicate conversions`, async _test => {
    const _tab = await _page(_test, _name);
    await _tab.evaluate(() => {
      for (const [_id, _value] of [['input-area', 'id\n7'], ['pg-table-name', 'autofilled'], ['number-offset', '3']]) {
        const $control = document.getElementById(_id);
    ...
    const _calls = await _tab.evaluate(() => {
      ...
      let _calls = 0;
      window._convertNumberAdjustment = (..._args) => { _calls += 1; return _convert(..._args); };
      const $input = document.querySelector('#input-area');
      $input.value = 'id\n8';
  ...
  test(`${_name}: R06 saving one preference does not resurrect externally deleted text`, async _test => {
    ...
    ASSERT.deepEqual(await _tab.evaluate(_key => JSON.parse(localStorage.getItem(_key)), STORAGE_KEY), { theme: 'dark' });
    ...
    ASSERT.deepEqual(await _tab.evaluate(_key => JSON.parse(localStorage.getItem(_key)), STORAGE_KEY), { theme: 'dark', input: 'explicitly edited again' });
    await _tab.reload();
  ...
  test(`${_name}: R07 page restoration reuses valid results without losing output repair or selection`, async _test => {
    ...
    const _state = await _tab.evaluate(() => {
      ...
      for (const [_name, _key] of [['_getLines', 'lines'], ['_parseTabularInput', 'table'], ['_convertNumberAdjustment', 'number']]) {
        const _original = window[_name];
        window[_name] = (..._args) => { _calls[_key] += 1; return _original(..._args); };
      }
      const _start = performance.now();
      ...
      window.dispatchEvent(new Event('pageshow'));
      return { unchangedCalls: _unchangedCalls, calls: _calls, repaired: _repaired, selection: [$input.selectionStart, $input.selectionEnd, $input.selectionDirection], elapsed: _elapsed, changed: $output.value.includes('INSERT INTO "new_table"') };
    });
  ...
  test(`${_name}: R09 validity changes are exposed once and incomplete native numbers still recover`, async _test => {
    ...
    for (const [_id, _invalid, _valid] of [['pg-table-name', 'bad;', 'records'], ['number-offset', '1.5', '-2']]) {
  ...
  test(`${_name}: R10 mixed edits, copies and page restoration retain consistent results and persistence`, async _test => {
    ...
    const _structure = await _tab.locator('body *').evaluateAll(_elements => _elements.map($element => $element.tagName));
    for (let _step = 0; _step < 18; _step += 1) {
      ...
      await _tab.evaluate(({ _input, _offset, _table, _step }) => {
        const _eventType = _step % 2 ? 'change' : 'input';
        for (const [_id, _value] of [['input-area', _input], ['number-offset', _offset], ['pg-table-name', _table]]) {
          const $control = document.getElementById(_id);
          $control.value = _value;
          if (_step % 3 !== 0) $control.dispatchEvent(new Event(_eventType, { bubbles: true }));
        }
        if (_step % 3 === 0) {
          document.querySelectorAll('.output-area').forEach($output => { $output.value = 'stale restored'; });
          window.dispatchEvent(new Event('pageshow'));
        }
      }, { _input, _offset, _table, _step });
      const _values = await _tab.locator('.output-area').evaluateAll(_outputs => _outputs.map($output => $output.value));
      ...
      if (_step % 6 === 5) {
        await _tab.reload();
        ASSERT.deepEqual(await _tab.locator('.output-area').evaluateAll(_outputs => _outputs.map($output => $output.value)), _values);
      }
    }
    ASSERT.deepEqual(await _tab.locator('body *').evaluateAll(_elements => _elements.map($element => $element.tagName)), _structure);
  ...
  test(`${_name}: R11 table-name edits reuse prepared SQL without rewriting cell values`, async _test => {
    ...
    const _result = await _tab.evaluate(() => {
      ...
      window._parseTabularInput = (..._args) => { _parses += 1; return _parse(..._args); };
      window._formatPgValue = (..._args) => { _formats += 1; return _format(..._args); };
      const _start = performance.now();
      ...
      return { parses: _parses, formats: _formats, elapsed: performance.now() - _start,
  ...
  test(`${_name}: R12 copying eventless edits persists both immediate and pending changes`, async _test => {
    const _tab = await _page(_test, _name, () => {
      window._pending = [];
      Object.defineProperty(navigator, 'clipboard', { value: { writeText: () => new Promise(_resolve => window._pending.push(_resolve)) } });
    });
    // Let the initial pageshow save finish so it cannot mask a missing copy save.
    await _tab.locator('#input-area').fill('previously saved');
    ...
    await _tab.waitForFunction(_key => JSON.parse(localStorage.getItem(_key))?.input === 'copied without input', STORAGE_KEY, { timeout: 2000 });
    await _tab.evaluate(() => {
      document.querySelector('#input-area').value = 'changed while pending';
      window._pending.shift()();
    });
    await _tab.waitForFunction(_key => JSON.parse(localStorage.getItem(_key))?.input === 'changed while pending', STORAGE_KEY, { timeout: 2000 });
    ASSERT.equal(await _tab.locator('.copy-btn.copied').count(), 0);
    await _tab.reload();
  ...
  test(`${_name}: R15 prepared SQL retains only the latest input and recovers from invalid tables`, async _test => {
    ...
    const _result = await _tab.evaluate(() => {
      ...
      const _outputs = [window._convertPostgreSQLBlock(_badInput, 'one'), window._convertPostgreSQLBlock(_badInput, 'two')];
